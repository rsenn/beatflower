...
typedef struct {
  ...
  UINT		id;				// depends on context
  UINT		parentId;		// 0 = root, or ML_TREEVIEW_ID_*
...
#define ML_IPC_DB_ADDORUPDATEFILE 0x0711      // Adds or Updates File in the Local MEdai Data Base.
...
void freeRecordListW(itemRecordListW *obj);
void emptyRecordListW(itemRecordListW *obj); // does not free Items
void freeRecordW(itemRecordW *p);

char *getRecordExtendedItemW(const itemRecord *item, const wchar_t *name);
void setRecordExtendedItemW(itemRecord *item, const wchar_t *name, wchar_t *value);

void allocRecordListW(itemRecordListW *obj, int newsize, int granularity
#ifdef __cplusplus
=1024
#endif
);
