...
#define lisdigit(x) (IsCharAlphaNumeric(x) && !IsCharAlpha(x))
...
static int register_var(compileContext *ctx, char *name, double **ptr)
{
  ...
  if (!nameptr[0])
  {
    lstrcpyn(nameptr,name,NSEEL_MAX_VARIABLE_NAMELEN);
  }
...
int nseel_setVar(compileContext *ctx, int varNum)
{
  if (varNum < 0) // adding new var
  {
    ...
    if (!strnicmp(var,"reg",3) && strlen(var) == 5 && lisdigit(var[3]) && lisdigit(var[4]))
    {
      int i,x=atoi(var+3);
      if (x < 0 || x > 99) x=0;
...

  {
    ...
    if (!nameptr[0]) 
    {
      lstrcpyn(nameptr,ctx->lastVar,NSEEL_MAX_VARIABLE_NAMELEN);
    }  
    return varNum;  
  }
...
int nseel_lookup(compileContext *ctx, int *typeOfObject)
{
  ...
  if (!strnicmp(ctx->yytext,"reg",3) && strlen(ctx->yytext) == 5 && lisdigit(ctx->yytext[3]) && lisdigit(ctx->yytext[4]) && (i=atoi(ctx->yytext+3))>=0 && i<100)
  {
